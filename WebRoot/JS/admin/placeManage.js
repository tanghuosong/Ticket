$(function(){
	$('#dg').datagrid({    
		...
		loadMsg: "数据加载中...",
		idField:"id",
	    url:'car_admin_queryPlaceByPage.action?type=id'+'&&key=', 
	    columns:[[    
			...
			handler: function(){
            	...
            	if(rows.length==1){
	            	parent.$('#win').window({ 
	            	    ...
	            	    width:400,    
	            	    height:350,  
	            	    content:"<iframe src='send_admin_updatePlace.action' frameborder='0' width='100%' height='100%'/>",
	            	    modal:true
	            	});
            	}else {
            		$.messager.show({
			...
			handler: function(){
		         ...
		         if(rows.length!=0){
		        	 ...
		        	 $.messager.confirm('温馨提示', '确定删除吗？', function(r){
		        			if (r){
		        				$.post("car_admin_deletePlaceByIds.action", { ids: ids },
		        					function(data){
		        					 $.messager.show({
				            			title:'温馨提示',
				            			msg:'删除成功！',
